...
i32 main(int argc, const char *argv[]) {
  ...
  StartBuild();
  {
    Executable rpnmath = CreateExecutable((ExecutableOptions){
      .output = "rpnmath",
      .std = args.stdlevel,
    ...
    AddIncludePaths(rpnmath, "./include");
    AddFile(rpnmath, "./src/*.c");
    if (isLinux()) {
      LinkSystemLibraries(rpnmath, "m");
    }
    InstallExecutable(rpnmath);
  }
  EndBuild();
